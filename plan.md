# After — Convex All Gas Hackathon build plan

> **One line:** Grief admin: finds each company's real bereavement procedure, drafts and sends the notification letters from one inbox, and tracks every reply on a calm shared board.
>
> **Repo:** `hack-gas2` · **AgentMail account:** A · **Event:** Convex All Gas Hackathon · **Deadline:** September 22, 2026, 12:00 PM PT · **Stack:** Convex (backend + static hosting on `*.convex.site`), React + Vite + Tailwind, any OpenAI-compatible LLM endpoint (env-configured; described as "OpenAI" in judge-facing docs), Firecrawl, AgentMail.
>
...
## 3. Product spec — After (closing a life's accounts, gently)

### 3.1 The pitch
When someone dies, the person left behind has to notify forty companies — bank, phone, utilities, streaming, insurance, social media — each with its own bereavement process buried on a support page, each demanding a different document, each needing a letter. **After** does the admin so the grieving person doesn't have to: name the companies, and it finds each company's actual bereavement procedure, drafts the notification letters in a calm voice, sends them from a dedicated inbox, tracks every reply, and nudges when a company goes quiet. Emotional payoff: a checklist quietly turning from grey to green, and a screen that says "You've done enough for today." Judges will remember it.

### 3.2 Core loop
Create an estate case (first name of the person, date, your relationship — minimal PII) → add companies (from a category checklist or free text) → **Firecrawl** `search` + `scrape` finds each company's "deceased customer / bereavement / close account" page and the **LLM** extracts a *playbook* (steps, documents required, contact channel, expected timeline) → the LLM drafts a notification letter per company that the user previews and edits (human-in-the-loop) → **AgentMail** sends each from the case inbox → replies arrive via webhook and are classified (`closed | needs_documents | needs_call | auto_reply | other`), moving the card on the board → a **cron** sweeps for companies silent >7 days and drafts a polite follow-up for one-click approval → invite a sibling by share link; the board is live for everyone.

### 3.3 Data model
- `estates`: ownerId, slug, personFirstName, dateOfPassing, relationship, caseCode (short unique code for email routing, see §4.3), createdAt. Index by ownerId, slug, caseCode.
- `estateMembers`: estateId, userId, role. Index by estateId, by userId.
- `playbooks` (shared cache across users): companyKey (normalized name), companyName, sourceUrl, steps[], documents[], contactEmail?, contactFormUrl?, phone?, expectedDays?, rawExcerpt, model, fetchedAt. Index by companyKey.
- `tasks`: estateId, companyName, companyKey, category, status (`researching|draft_ready|sent|awaiting|needs_documents|needs_call|closed|bounced`), playbookId?, draftSubject?, draftBody?, sentMessageId?, sentAt?, lastInboundAt?, nextFollowUpAt?, notes?. Index by estateId+status, by estateId.
- `messages`: estateId, taskId?, direction, messageId, threadId, subject, extractedText, fullText, classification?, summary?, receivedAt. Index by messageId, by estateId, by threadId.
- `documents`: estateId, name (e.g. "Death certificate"), storageId?, status (`have|need`). Index by estateId. (Demo uses a dummy PDF; never store real certificates in testing.)
- `dailyPlan`: estateId, date, taskIds[] — "today's three things" chosen by the LLM.

### 3.4 Convex functions
- `estates.create` (assigns a caseCode), `estates.get`, `estates.publicShare` (slug-joined members), `estates.join`.
- `tasks.addMany` (mutation; schedules `research.lookup` per company), `tasks.updateDraft`, `tasks.approveAndSend`, `tasks.setStatus`.
- `research.lookup` (internalAction): check `playbooks` cache by companyKey; else Firecrawl `search("<company> deceased account holder close account bereavement")` → pick top 2 results (LLM chooses relevant ones) → `scrape` → `extract()` playbook → insert; then `ai.draftLetter`.
- `ai.draftLetter` (internalAction): system prompt = calm, brief, factual, no euphemisms, includes what the playbook says is required; inserts draftSubject/body, status `draft_ready`.
- `mail.send` (internalAction via `sendMail()`), attaches selected documents from storage when the playbook requires them.
- `http.ts` webhook → `mail.ingest` → `ai.classifyInbound` → update task status + `summary`, set `nextFollowUpAt` when awaiting.
- `crons.ts`: daily `followups.sweep` — tasks in `awaiting` past `nextFollowUpAt` get an LLM-drafted follow-up (status `draft_ready` with a `followUp` flag); daily `plan.build` — LLM picks three tasks for today per estate.
- Agent component: "Ask After" side panel — an Agent with the estate's playbooks/tasks as context, for "what does the bank need from me?" questions. Threads stored via the component.

### 3.5 Integration specifics
- **Firecrawl:** `search` with `scrapeOptions.formats: ["markdown"]` so results include content; `scrape` the chosen page(s). Cache playbooks by company — the second user who adds "Netflix" gets it instantly (show "verified from netflix.com 3 days ago").
- **LLM:** playbook schema `{steps: string[], documents: string[], contactEmail?: string, contactFormUrl?: string, phone?: string, expectedDays?: number, confidence: number}`; letter drafting; inbound classification `{classification, summary, requestedDocuments?: string[]}`.
- **AgentMail:** single app inbox (§4.3) with `[AFT-xxxx]` codes in subjects; send with attachments; `message.bounced` → status `bounced` with a "try the web form instead" link from the playbook.

### 3.6 UI
- Palette: soft, low-saturation, generous whitespace, serif headings. No red. No confetti. Micro-copy matters ("You don't have to finish this today.").
- **/start** three fields, one button.
- **/e/:id** the board: left = category checklist to add companies (Bank, Phone, Utilities, Streaming, Social, Insurance, Government, Other + free text); center = task cards in columns *Researching → Ready to send → Waiting → Needs something → Done*, cards move live; a card expands to show the playbook (steps, documents, source link), the draft (editable), and the thread; right = *Today* panel with three things and a progress ring, plus *Ask After*.
- **/e/:id/share** invite link for a sibling; presence dots of who's viewing (Convex presence pattern or a simple heartbeat table).
- **Print view** of the whole plan for people who prefer paper.

### 3.7 Demo storyboard (≤ 3:00)
1. (0:00) "My dad died in March. Here's what the next month looked like." Open URL, create estate "Robert", add 6 companies from checklist + "Rogers". (0:30)
2. Cards appear in *Researching*; within seconds playbooks fill in — open Netflix's: 3 steps, documents, source link to the real page (Firecrawl). (1:00)
3. Card moves to *Ready to send*; open the draft letter; tweak one line; click *Send*. Show it in *Waiting*. (1:30)
4. Phone: reply as the company ("Thank you. Please send a copy of the death certificate to proceed."). Card slides to *Needs something* with a summary and the document chip "Death certificate". Attach the (dummy) document, resend. (2:15)
5. Reply "Account closed." Card moves to *Done*; the progress ring ticks; Today panel: "2 of 3 done. That's enough for today." Show share link + URL. (2:50)

### 3.8 Fixtures & test plan
Seed an estate with 8 tasks in mixed states and cached playbooks for 6 well-known companies (real pages, scraped once by the seed action so the demo is instant). Test bounce handling by emailing an invalid address with the override off *only on a throwaway inbox* — or simulate via a fixture.

### 3.9 Stretch
Government checklists by country (crawl official "what to do when someone dies" pages); calendar export of expected timelines; a "memorial mode" that hides the board on weekends.

### 3.10 Risks
Search results for "close account deceased" can be forums — have the LLM reject non-official domains (must match company domain). Keep PII minimal and say so in the UI and `hackathon.md`.
