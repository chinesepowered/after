...
export const unrouted = query({
  ...
  handler: async (ctx) => {
    ...
    // nothing; only an account with an email (i.e. real password sign-in, not
    // the anonymous provider) may read this, and bodies are truncated and
    // sender addresses redacted even then.
